...
<h3>Data Panen {{ $data->nama_petani }}</h3>
@endslot

@slot('action')
<li>
  <a href="{{ url('/panen/create/'.$data->id) }}">
    <i data-feather="plus"></i>
    <span class="me-3">Tambah Panen</span>
  </a>
</li>
@endslot
        ...
        <div class="col-sm-12">
          <form method="get">
            <div class="form-group">
              ...
              <input type="month" class="form-control" name="tanggal_panen"> 
            </div>
...
@push('scripts')
<script>
let harvest = {!! json_encode($getHarvest) !!}
...
map.on('load', () => {
  ...
  for(const element of harvest) {
    var el = document.createElement('div');
    el.className = 'marker';
    el.style.backgroundImage = `url('{{ asset('uploads/${element.plant.commodity.icon}') }}')`;
    el.style.width = '50px';
    el.style.height = '45px';
    el.style.backgroundRepeat = 'no-repeat';
    new mapboxgl.Marker(el)
      .setLngLat([element.plant.longitude, element.plant.latitude])
      .setPopup(
        new mapboxgl.Popup({
          offset: 25,
        })
        ...
        <table>
            <thead>
              <tr>
                ...
                <th>:</th>
                <th>${element.plant.commodity.nama_komoditas}</th>
              </tr>
              <tr>
                <th>Tanggal Panen</th> 
                <th>:</th>
                <th>${element.tanggal_panen}</th>
              </tr>
              <tr>
                <th>Jumlah Panen</th> 
                <th>:</th>
                <th>${element.jumlah_panen} Ton</th>
              </tr>
              <tr>
                <th></th>
                <th></th>
                <th><a class="btn btn-sm btn-primary"
                  onclick="openForm('{{ url('/panen/edit/${element.id}') }}', 'edit')">Ubah</a>
                </th> 
              </tr>
            </thead>
          </table>
